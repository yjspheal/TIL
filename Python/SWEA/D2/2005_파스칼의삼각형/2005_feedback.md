...
- [총평](#총평)
- [보완점](#보완점)
  - [1. 알고리즘 버그(포인터가 현재 줄을 참조) 수정](#1-알고리즘-버그포인터가-현재-줄을-참조-수정)
  - [2. 메모리 O(N)로 최적화 (in-place, 오른쪽→왼쪽 업데이트)](#2-메모리-on로-최적화-in-place-오른쪽왼쪽-업데이트)
  - [3. 조합식(nCk)로 직접 생성 (정확, 깔끔)](#3-조합식nck로-직접-생성-정확-깔끔)
- [최종 코드 예시](#최종-코드-예시)
