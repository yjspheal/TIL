...
def create_user(users):
    ...
    for user in users:
        if is_validation(user) == True:     # 반환 받은 값이 True인 경우 pass
            pass
    
        elif is_validation(user) == 'blocked':    
            error_count += 1
            continue                    # block이면 user_list에 추가하지 않아야함

    
        else:     # 반환 받은 값이 (False, error_list) 인 경우
            user = None    # 잘못된 데이터에는 None을 할당하여 데이터를 생성한다
            error_count += 1
            continue
...
def is_validation(user_info):
    ...
    error_list = []
    if blood_group not in blood_types:          # 혈액형이 blood group에 없으면
        # print(blood_group)
        error_list.append('blood_group')

    if company in black_list:                   # 회사가 blacklist 에 있으면 blocked 후 바로종료
        # print(company)
        return 'blocked'
        
    if '@' not in mail:                             # 메일에 @가 없으면
        # print(mail)
        error_list.append('mail')
        
    if len(name) < 2 or len(name) > 30 :         # 이름이 2자 미만 30자 초과라면
        # print(name)
        error_list.append('name')
        
    # website 키가 없거나, 있어도 사이트가 0개면
    if 'website' not in user_info.keys() or len(user_info['website']) == 0:                   
        # print('website')
        error_list.append('website')
        


    if error_list:      # error가 있으면
        return False, error_list        # tuple 반환
